...
from tests.TC_PNGK import TC_PNGK


class TC_PNEK(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        driver = cls.driver
        TC_PNEK.nibar = os.getenv("nibar")  # Set nibar on TC_PNEK
        time.sleep(3)
        driver.get(f"{cls.url}pages.php?Pg=pengembalianGedung")
        time.sleep(1)
        cls.shared = {}
        cls.tc_pngk = TC_PNGK()
        cls.tc_pngk.driver = cls.driver
    ...
    def setUp(self):
        if self._testMethodName in ["test_TC_PNEK_001","test_TC_PNEK_002"]:
            filter_pengamanan(self.driver, TC_PNEK.nibar or "")
            time.sleep(1)
            checkbox(
                self.driver, identifier=-1, by="index", table_selector="table.koptable"
            )
            time.sleep(1)
            href_button(self.driver, "javascript:pengembalianGedung.formEdit()")
            time.sleep(2)
    ...
    def test_TC_PNEK_001(self):
        print("test_TC_PNEK_001")
        driver = self.driver
    ...
    def test_TC_PNEK_002(self):
        print("test_TC_PNEK_002")
        driver = self.driver
        tc_pngk = self.tc_pngk
...
if __name__ == "__main__":
    unittest.main(defaultTest="TC_PNEK")
